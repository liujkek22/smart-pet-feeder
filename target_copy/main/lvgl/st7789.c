...
#define LCD_CLK 17   // lcd的clk引脚s
#define LCD_MOSI 46  // lcd的sda引脚
#define LCD_MISO -1 // lcd的miso引脚

#define LCD_DC 12                                  // lcd的dc引脚
...
void st7789_init(void)
{
    ...
    // 获取当前屏幕大小
    uint32_t hres = lv_disp_get_hor_res(NULL);  // 获取屏幕的水平分辨率
    uint32_t vres = lv_disp_get_ver_res(NULL);  // 获取屏幕的垂直分辨率
    // 旋转屏幕，指定旋转角度为 90 度
    lv_disp_set_rotation(NULL, LV_DISP_ROT_90);
    // 打印当前的屏幕大小
    printf("Screen size after rotation: %ld x %ld\n", vres, hres);
    printf("Current screen color depth: %d bits\n", LV_COLOR_DEPTH);
