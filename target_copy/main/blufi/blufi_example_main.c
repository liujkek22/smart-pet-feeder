                             ...
                             int32_t event_id, void *event_data)
{
    ...
    case IP_EVENT_STA_GOT_IP:
    {
        ...
        {
            // 发送wifi连接成功信息
            esp_err_t ret = esp_blufi_send_wifi_conn_report(mode, ESP_BLUFI_STA_CONN_SUCCESS, softap_get_current_connection_number(), &info);
            if (ret != ESP_OK)
            {
                BLUFI_INFO("BLUFI send wifi conn report failed\n");
            }
            else
            {
                // 反初始化bulufi，关闭资源
                ...
                if (ret == ESP_OK)
                {
                    ESP_LOGW("deint", "BLUFI host deinit suc");
                }
                ret = esp_blufi_controller_deinit();
                if (ret == ESP_OK)
                {
                    ESP_LOGW("deint", "BLUFI controller deinit suc");
                }
            }
        }
        else
